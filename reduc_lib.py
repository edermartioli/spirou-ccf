...
def reduce_spectra(spectra, nsig_clip=0.0, combine_by_median=False, subtract=True,  output="", fluxkey="fluxes", fluxerrkey="fluxerrs", wavekey="wl", update_spectra=False, plot=False, verbose=False) :
    ...
    for order in range(49) :
        ...
        order_template = calculate_template(fluxes, wl=spectra[wavekey][order], fit=True, median=combine_by_median, subtract=subtract, sub_flux_base=sub_flux_base, verbose=False, plot=plot)
...
#################################################################################################
def calculate_template(flux_arr, wl=[], fit=False, median=True, subtract=False, sub_flux_base=1.0, verbose=False, plot=False, pfilename=""):
    """
