...
def generate_collection(inputdata, with_tfits_only=True, verbose=False) :
    """
    ...
    for i in range(len(inputdata)) :
        try :
            ...
            if inputdata[i].endswith("e.fits") :
                tfile = inputdata[i].replace("e.fits", "t.fits")
                if not os.path.exists(tfile) :
                    if with_tfits_only :
                        print("WARNING: could not find telluric corrected for file {0}, skipping ...".format(inputdata[i]))
                        continue
                    else :
                        print("WARNING: could not find telluric corrected spectrum for file {0}, using e.fits instead, quality of CCF is compromised".format(inputdata[i]))
                        tfile = inputdata[i]
...
def run_ccf(tfile, sci_mask, efile='', fp_mask='', run_calib=True, plot=False, verbose=False) :
    ...
    sci_ccf = ccf_lib.run_ccf_new(ccf_params, spectrum, valid_orders=order_subset_for_mean_ccf, plot=plot, interactive_plot=False, merge_headers=True)
    ...
    if run_calib :
        ...
        calib_ccf = ccf_lib.run_ccf_new(ccf_fp_params, spectrum_fp, science_channel=False, plot=plot, merge_headers=True)

        return sci_ccf, calib_ccf
    else :
        return sci_ccf
...
parser.add_option("-t", action="store_true", dest="withouttfiles", help="accept files without respective t.fits", default=False)
...
collections, objtemps, tfiles, arg_max_snr = generate_collection(inputdata, verbose=True)
...
for object in collections['object'] :
    ...
    abs_path_of_ref_spectrum = os.path.abspath(tcorr_spectra[refexp])
    ...
    ref_sci_ccf = run_ccf(tcorr_spectra[refexp], mask_file, run_calib=False, plot=options.plot)

    # Set systemic velocity to the RV measured on the reference exposure
    rv_sys = ref_sci_ccf["header"]['RV_OBJ']

    if options.verbose :
        print("Reference spectrum {0} observed on {1} showing RV_sys={2:.5f} km/s".format(tcorr_spectra[refexp], ref_sci_ccf["header"]["DATE"], rv_sys))
    ...
    bjd, rv, rverr, fprv, fprverr = [], [], [], [], []
    sci_ccf_file_list = []
    snr = []

    for i in range(len(file_list)) :
        if options.verbose :
            print("Running CCF on file {0}/{1}: efits={2} tfits={3}".format(i,len(file_list)-1,os.path.basename(file_list[i]),os.path.basename(tcorr_spectra[i])))

        sci_ccf, calib_ccf = run_ccf(tcorr_spectra[i], mask_file, file_list[i], fp_mask)

        if options.verbose:
            print("Spectrum: {0} DATE={1} SNR={2:.0f} Sci_RV={3:.5f}km/s FP_RV={4:.5f}km/s".format(os.path.basename(tcorr_spectra[i]), sci_ccf["header"]["DATE"],sci_ccf["header"]["SPEMSNR"], sci_ccf["header"]['RV_OBJ'], calib_ccf["header"]['RV_OBJ']))
        ...
        fprv.append(calib_ccf["header"]['RV_OBJ'])
        fprverr.append(calib_ccf["header"]['CCFMRVNS'])
        snr.append(sci_ccf["header"]["SPEMSNR"])

    bjd = np.array(bjd)
    rv, rverr = np.array(rv), np.array(rverr)
    fprv, fprverr = np.array(fprv), np.array(fprverr)
    snr = np.array(snr)
    
    # Set path and filename for output template
    rv_file = outdir + "/{0}_rv.rdb".format(object)
    fprv_file = outdir + "/{0}_fp.rdb".format(object)
    if options.verbose :
        print("Saving output RVs in the files: {0}, {1}".format(rv_file, fprv_file))
    spiroulib.save_rv_time_series(rv_file, bjd, rv, rverr)
    spiroulib.save_rv_time_series(fprv_file, bjd, fprv, fprverr)

    if options.verbose :
        print("Building template spectrum out of {0} spectra ...".format(len(tcorr_spectra)))

    try :
        # Build template spectrum for object:
        template_spectrum = spiroulib.template_using_fit(tcorr_spectra, rv_file, median=True, normalize_by_continuum=True, verbose=False, plot=options.plot)
        ...
        if options.save_template :
            ...
            spiroulib.write_spectrum_to_fits(template_spectrum, template_output, header=fits.getheader(tcorr_spectra[refexp]), wavekey='wl_template', fluxkey='flux_template', fluxerrkey='fluxerr_template')
    except :
    ...
    sanit, drs_version = False, fits.getheader(tcorr_spectra[refexp])['VERSION']
    ...
    tbl = ccf2rv.get_object_rv(sci_ccf_file_list, collection_key=collection_key, method="all", exclude_orders=exclude_orders, snr_min=snr_min, bandpass=bandpass, save_rdb_timeseries=True, save_csv_table_of_results=True, save_ccf_cube=False, save_weight_table=False, doplot=options.plot, showplots=options.plot, saveplots=options.plot, verbose=options.verbose)
